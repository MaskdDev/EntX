...
encryption_style = """aYb1AcX8dWe4fB!IUg-L0{hi:,;/\CjTk2Hl&V_m7@n"DMo^pJ)5q E#Nr>+st9$FR*"Q=ZuP<Kv(%O3w}GxS6yz"""
...
def encryptkit(to_encrypt: str, password: str):
    ...
    for char in str(to_encrypt):
        if char in encryption:
            ...
            new_char_position = (char_position + len(password) * ord(encryption[len(password)]) + index) % 88
...
def decryptkit(to_decrypt: str, password: str):
    ...
    for char in str(to_decrypt):
        if char in encryption:
            char_position = encryption.find(char)
            old_char_position = (char_position - len(password) * ord(encryption[len(password)]) - index) % 88
